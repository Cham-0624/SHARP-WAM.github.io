...
`<g id="query-group" transform="translate(0 -28)">`+box(648,36,316,100,'#fff','#414141',5,'stroke-width="2"')+tx(669,64,'Current Q',22,C.ink,'start','font-weight="650"')+tx(669,80,'Representative MoT queries · vary by layer & control step',11,C.muted,'start')+tx(943,61,'Hover to switch',13,C.muted,'end','id="query-hint"')+[0,1,2].map(q=>`<g class="query-choice" data-query="${q}" tabindex="0" role="button" aria-label="Query ${q+1}">`+box(663+q*97,85,90,38,'#fff','#9eacb8',4,`id="query-bg-${q}"`)+tx(708+q*97,112,'Q'+(q+1),22,C.ink,'middle','font-weight="600"')+'</g>').join('')+'</g>'+`<g id="route-lines" class="trace" pointer-events="none"></g><g id="raw-paths" class="trace" pointer-events="none"></g><g id="incoming" pointer-events="none"></g><g id="reading" pointer-events="none"></g></g><g id="auto-packets" pointer-events="none" aria-hidden="true"></g>`;
 ...
 $('scope').textContent='Colors, A/B/C/D entries, six similarity scores and final EMA utilities follow the paper figure. Capacity 3 and Top-2 illustrate the mechanism; Q1–Q3 are representative MoT query patterns, which vary across layers and control steps. Query selections, probe aggregation paths and intermediate EMA trajectories are schematic; probes are not assigned one-to-one to camera blocks. 120 tokens / eight blocks per frame; anchor/recent paths omitted.';$('touch-note').textContent='Tap and swipe on touch screens, or focus with Tab.';paintQuery();update();paintPlayback();if(mode==='auto'){paintAutoStage();paintAutoPackets();}}
